#!/usr/bin/env sh
# sv_gate_count.sh — Estimate gate count for a SystemVerilog module using Yosys
...
if ! command -v yosys > /dev/null 2>&1; then
    ...
    printf "  Install: ${CYAN}sudo apt install yosys${RESET}  /  ${CYAN}brew install yosys${RESET}\n"
    printf "  Or grab OSS CAD Suite: ${CYAN}https://github.com/YosysHQ/oss-cad-suite-build${RESET}\n"
    exit 1
fi
...
if [ -z "$TOP_MODULE" ]; then
    # Try grep -P (perl regex), fall back to basic grep
...
# ─── Build Yosys script ──────────────────────────────────────────────────────────
{
    echo "# Auto-generated by sv_gate_count.sh"
    echo "read_verilog -sv $ABS_FILE"
    ...
    if [ -n "$LIBERTY_FILE" ] && [ -f "$LIBERTY_FILE" ]; then
        ...
        echo "# Generic gate mapping (no liberty file provided)"
        echo "techmap"
        echo "opt"
        echo "abc -g NAND"
        echo "stat -tech cmos"
...
if [ -z "$STAT_BLOCK" ]; then
    # Try without strict spacing in case of formatting variation
    STAT_BLOCK=$(grep -A 40 "$TOP_MODULE" "$YOSYS_LOG" | grep -A 40 "Number of" | head -50 || true)
fi
...
# stat -tech cmos prints "NAND2-equivalent" directly
NAND2_EQ=$(grep -i 'NAND2-equivalent' "$YOSYS_LOG" | grep -oE '[0-9]+\.[0-9]+' | tail -1 || true)

# Fallback: use raw cell count
if [ -z "$NAND2_EQ" ]; then
    NAND2_EQ=$(grep 'Number of cells:' "$YOSYS_LOG" | tail -1 | grep -oE '[0-9]+$' || true)
    LABEL="raw cell count (run with stat -tech cmos for NAND2 equiv)"
else
    LABEL="NAND2-equivalent gates (cmos weighting)"
fi
